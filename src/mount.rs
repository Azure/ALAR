...
pub(crate) fn mkdir_assert() -> Result<(), io::Error> {
    match fs::create_dir_all(constants::ASSERT_PATH) {
        Ok(()) => Ok(()),
        Err(e) => {
            println!("Error while creating the assert directory: {e}");
            Err(e)
        }
    }
}
...
pub(crate) fn mkdir_rescue_root() -> Result<(), io::Error> {
    match fs::create_dir_all(constants::RESCUE_ROOT) {
        Ok(()) => Ok(()),
        Err(e) => {
            println!("Error while creating the rescue-root directory: {e}");
            Err(e)
        }
    }
}
fn mount(source: &str, destination: &str, _option: Option<&str>) {
    ...
    match process::Command::new("modprobe").arg("xfs").status() {
        Ok(_) => {}
        Err(_) => helper::log_error(
            "Loading of the module xfs was not possible. This may result in mount issues! : ",
        ),
    }
